...
export default function ORPData() {
    ...
    useEffect(() => {
        fetch(`/api/ORPData?sorted=true&time=${time}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                setCurrentGraphData(formatData(data.length > 0 ? data[index] : [], 'ORP'));
                setLoading(false);
            })
    ...
    const handleClick = (direction) => {
        ...
        if (direction === "next"){
            ...
            setCurrentGraphData(formatData(fullData[next], "ORP"));
            setLoading(false);
        }else{
            if (fullData.length === 1){
                setLoading(false);
