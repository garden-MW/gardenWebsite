...
export default function BlankData3() {   //change to appropriate name
    ...
    useEffect(() => {
        fetch(`/api/blankData3?sorted=true&time=${time}`)   //replace blankData3 with appropriate name (will also need to update the api route name)
            .then(response => {
                if (!response.ok) {
        ...
        <div className=" p-10 flex flex-col h-screen w-screen space-y-5 items-center justify-evenly overflow-scroll">
            ...
            <div className="w-full h-auto lg:max-w-[50%] flex justify-center items-center">
                ...
                <div className="w-full h-auto lg:max-w-[90%] flex justify-center items-center">
                    <SpecificGraph type="blank3" data={currentGraphData} setTime={setTime} /> {/*replace blank3 with appropriate name*/}
                </div>
