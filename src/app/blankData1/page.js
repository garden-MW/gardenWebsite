...
export default function BlankData1() {   //change to appropriate name
    ...
    useEffect(() => {
        fetch(`/api/blankData1?sorted=true&time=${time}`)   //replace blankData1 with appropriate name (will also need to update the api route name)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                setCurrentGraphData(formatData(data[index], 'Blank1')); //replace Blank1 with appropriate name
                setLoading(false);
            })
      ...
      const handleClick = (direction) => {
        ...
        if (direction === "next"){
            ...
            setCurrentGraphData(formatData(fullData[next], 'Blank1')); //replace Blank1 with appropriate name
            setLoading(false);
        }else{
            let next = 0; 
        ...
        <div className=" p-10 flex flex-col h-screen w-screen space-y-5 items-center justify-evenly overflow-scroll">
            ...
            <div className="w-full h-auto lg:max-w-[90%] flex justify-center items-center">
                ...
                <div className="w-full h-auto lg:max-w-[50%] flex justify-center items-center">
                    <SpecificGraph type="blank1" data={currentGraphData} setTime={setTime} /> {/*replace blank1 with appropriate name*/}
                </div>
            ...
            <div className="flex flex-col md:flex-row w-full h-auto md:justify-between ">
                <div className="w-full h-auto flex items-center mb-3 md:mb-0">
                    <RowInfo type={"blank1"} data={currentData} isAverage/> {/*replace blank1 with appropriate name*/}
                </div>
                <div className="w-full h-auto flex items-center">
                    <RowInfo type={"blank1"} data={currentData} isRecent/> {/*replace blank1 with appropriate name*/}
                </div>
            </div>
