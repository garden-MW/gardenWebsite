...
EC_ARCHIVE_FILE = '/home/hydro/raspberry_pi_scripts/ec_data/ec_data_archive.json'
SENSOR_NAME = "ec1"
...
if batch:
    ...
    if response.status_code == 200:
        ...
        # 1. Remove uploaded entries from main ec_data.json
        with open(EC_DATA_FILE, 'w') as f:
            json.dump([], f, indent=2)
            
        # 2. Save uploaded entries into an archive file
        if os.path.exists(EC_ARCHIVE_FILE):
            with open(EC_ARCHIVE_FILE, 'r') as f:
                try:
        ...
        with open(EC_ARCHIVE_FILE, 'w') as f:
            json.dump(archive_data, f, indent=2)
        print(f"Archived {len(to_upload)} entries to {EC_ARCHIVE_FILE}")
